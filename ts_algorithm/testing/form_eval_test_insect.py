...
def generate_stats(stats_ar: np.ndarray) -> pd.DataFrame:
    ...
    if (ts_1=='InsectEPGSmallTrain')&\
       (no_1==226):
        df_tmp = df_res[(df_res['ts_2']=='InsectEPGRegularTrain')&\
               (df_res['no_2']==56)]
        print(f"{ts_1} - {no_1}\ndf_res:\n {df_tmp}")
    return df_res
...
def main():
    ...
    df_sample = df_sample[(df_sample['ts_name']=='InsectEPGSmallTrain')&
              (df_sample['no']==226)]
    ts_l = list(df_sample.values) # convert dataframe to list of elements
    print(f"length of ts_l: {len(ts_l)}")
    ...
    df_res.to_csv("../data/insect_match.csv", index=False)
