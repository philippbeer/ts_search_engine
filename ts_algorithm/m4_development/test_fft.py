...
def full_fft(ts_name: str,
    ...
    if (ts_name == 'InsectEPGRegularTrain')&\
       (no == 56):
        PSD_l = list(PSD)
        idx_powerful_PSD = sorted(range(len(PSD_l)), key= lambda
                                      x: PSD_l[x])[-5:]
        print(f"PSDs {fft_freqs[idx_powerful_PSD]}")
...
def read_raw_ts(start_time: datetime.date,
    ...
    df_l2 = []
    for df in df_l:
        if df.loc[0,'name']=='InsectEPGRegularTrain':
            df_l2.append(df)
    
    time_series = []
    with Pool(processes=no_prc) as pool:
        for res in tqdm(pool.imap_unordered(separate_ucr_ts, df_l2),
                        total=len(df_l)):
            time_series += res
